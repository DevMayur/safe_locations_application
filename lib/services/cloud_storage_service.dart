...
class CloudStorageService {
  ...
  Future<String?> saveUserImageToStorage(String _uid, PlatformFile _file) async {
    try {
      Reference ref = _storage.ref().child("images/users/$_uid/profile.${_file.extension}");
  ...
  Future<String?> saveChatImageToStorage(String _chatID, String _userID, PlatformFile _file) async {
    try {
      Reference ref = _storage.ref().child("images/chats/${_chatID}/${_userID}_${Timestamp.now().millisecondsSinceEpoch}.${_file.extension}");
      UploadTask _task = ref.putFile(File(_file.path!));
      return await _task.then((_result) => _result.ref.getDownloadURL(),);
    } catch (e) {
      print(e);
